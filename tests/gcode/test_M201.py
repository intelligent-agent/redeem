...
class M201_Tests(MockPrinter):
    ...
    def exercise(self):
        ...
        return {"values": values, "call_args": self.printer.path_planner.native_planner.setAcceleration.call_args[0][0]}
    ...
    def test_gcodes_M201_all_axes_G21_mm(self):
        ...
        for i, axis in enumerate(self.printer.AXES):
          ...
          self.assertEqual(expected, result, axis+": expected {:.0f} but got {:.0f}".format(expected, result))
    ...
    def test_gcodes_M201_CoreXY(self):
        ...
        self.assertEqual(test_data["call_args"][0], test_data["call_args"][1], 
            "For CoreXY mechanics, X & Y values must match. But X={}, Y={} (mm/min / 3600)".format(
                test_data["call_args"][0],
                test_data["call_args"][1]
            )
        )
    ...
    def test_gcodes_M201_Delta(self):
        ...
        while True: # account for super, ultra-duper remote possibility of three equal random numbers for X , Y and Z
          test_data = self.exercise()
          if (test_data["values"]["X"] + test_data["values"]["Y"] + test_data["values"]["Y"]) != (test_data["values"]["X"] * 3):
            break
