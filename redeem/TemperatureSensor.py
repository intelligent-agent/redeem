...
class TemperatureSensor:
  ...
  def read_adc(self):
    ...
    try:
      ...
      Alarm(Alarm.THERMISTOR_ERROR, "Unable to get ADC value ({0}): {1}".format(
          e.errno, e.strerror))
  ...
  def __init__(self, pin, sensorConfiguration, name):
    ...
    if len(sensorConfiguration) != 5:
      Alarm(
          ...
          "Sensor configuration for {0} is missing parameters. Expected: 5, received: {1}.".format(
              pin, len(sensorConfiguration)))
    else:
      self.pin = pin
...
class Tboard(TemperatureSensor):
  def __init__(self, pin, sensorConfiguration, name):
    ...
    logging.debug("Initialized temperature sensor at {0} with temp/deg = {1}".format(
        pin, sensorConfiguration[1]))
