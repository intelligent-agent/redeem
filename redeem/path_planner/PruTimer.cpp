...
void PruTimer::pushBlock(uint8_t* blockMemory, size_t blockLen, unsigned int unit, uint64_t totalTime, SyncCallback* callback)
{
    ...
    for (unsigned int i = 0; i < nbBlocks; i++)
    {
...

        {
            ...
            {
                LOGINFO("PRU Queue is full by time - " << totalQueuedMovesTime << "/" << maxQueuedMovesTime << std::endl);
                logBlock--;
                logged = true;
            }
            ...
            if (logged)
            {
                LOGINFO("PRU Queue has space again" << std::endl);
            }
...
void PruTimer::run()
{
    ...
    while (!stop)
    {
        ...
        *ddr_nr_events = (*ddr_nr_events) + 1;
        ...
        {
            LOGINFO("PRU Queue underflow" << std::endl);
        }
