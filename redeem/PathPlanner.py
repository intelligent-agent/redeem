...
class PathPlanner:
  ...
  def _home_internal(self, axis):
    ...
    for a in axis:
      ...
      fine_search_speed = min(
          abs(speed), abs(self.printer.home_backoff_speed[Printer.axis_to_index(a)]))
      path_zero[a] = 0
  ...
  def autocalibrate_delta_printer(self, num_factors, simulate_only, probe_points, print_head_zs):
    if self.printer.axis_config != Printer.AXIS_CONFIG_DELTA:
      logging.warning("autocalibrate_delta_printer only supports " "delta printers")
      return

    params = delta_auto_calibration(Delta, self.center_offset, num_factors, simulate_only,
