...
int main(void) {
	...
	while(1)
	{
		...
		if ((state >> 0) & 1) { stepperMask |= STEPPER_MASK_X1;	}
		if ((state >> 1) & 1) { stepperMask |= STEPPER_MASK_Y1;	}
		if ((state >> 2) & 1) { stepperMask |= STEPPER_MASK_Z1;	}
		if ((state >> 3) & 1) { stepperMask |= STEPPER_MASK_X2;	}
		if ((state >> 4) & 1) { stepperMask |= STEPPER_MASK_Y2;	}
		if ((state >> 5) & 1) { stepperMask |= STEPPER_MASK_Z2;	}
